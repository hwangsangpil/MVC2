...
public class BusinessDAO {
	...
	//업체관리 수정 페이지 이동
		public BusinessDTO businessMod(int busiNum){
			...
			sql.append("AND BUSINESS_NUM=?										\n");
				...
				System.out.println("dao view수정페이지:"+pstmt.toString());
				rs = pstmt.executeQuery();
				if (rs.next()) {
					dto.setBusiNum(rs.getInt("BUSINESS_NUM"));
					dto.setBusiName(rs.getString("BUSINESS_NAME"));
	...
	//공고 상세보기 수정
		public int BusinessViewModOk(int busiNum, String busiOpening, String busiPercent, String busiPrice, String busiWay, String busiArea){
			ResultSet rs = null;
			PreparedStatement pstmt = null;
			Connection conn = null;
